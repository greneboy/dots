 ...
 */
	...
	#define POWER_SUPPLY_THRESHOLD			"/sys/class/power_supply/%s/charge_control_end_threshold"
	...
	{
		uintmax_t charge_now, current_now, charge_full, control_threshold, m, h;
		double timeleft;
		char path[PATH_MAX], state[12];
		...
		if (!strcmp(state, "Charging")) {
			...
			if (!pick(bat, POWER_SUPPLY_CHARGE_FULL, POWER_SUPPLY_ENERGY_FULL, path,
            	      sizeof(path)) ||
            	pscanf(path, "%ju", &charge_full) < 0)
            	return NULL;

			if (esnprintf(path, sizeof(path), POWER_SUPPLY_THRESHOLD, bat) < 0)
				return NULL;
			if (pscanf(path, "%d", &control_threshold) != 1)
				return NULL;

        	if (current_now == 0)
            	return NULL;

        	timeleft = ((double)charge_full * ((double)control_threshold / 100) - (double)charge_now) / (double)current_now;
        	h = timeleft;
        	m = (timeleft - (double)h) * 60;
